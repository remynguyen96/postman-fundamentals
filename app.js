...
function isAuthorized(req) {
  if (req.headers.authorization) {
    const user_and_password = new Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString();

    const user = user_and_password.split(':')[0];
    const pw = user_and_password.split(':')[1];

    return user === 'admin' && ((env === 'test' && pw === 'admin_test') || pw === 'admin');
  } else {
    return false;
  }
}

const bookRouteNeedsAuth = (req) => req.url.match(/books/) && (req.method === 'DELETE' || req.method === 'PUT');

const userRouteNeedsAuth = (req) => req.url.match(/users/) && req.method === 'DELETE';

server.use((req, res, next) => {
  if ((bookRouteNeedsAuth(req) || userRouteNeedsAuth(req)) && !isAuthorized(req)) {
    res.sendStatus(401);
  } else {
    next();
  }
...
server.post('/users', (req, res, next) => {
  ...
  const newWishlist = collection.insert({ name: `${req.body.firstName}'s List`, books: [] }).write();
...
const getBooksForWishlist = (wishlistId, db) => {
  const wishlist = db.defaults({ wishlists: [] }).get('wishlists').find({ id: wishlistId }).cloneDeep().value();

  const fullBooks = wishlist.books
    ? wishlist.books.map((b) => db.get('books').find({ id: b }).cloneDeep().value())
    : [];
...
server.get('/wishlists/:wishlistId/books', (req, res) => {
  const wishlist = getBooksForWishlist(parseInt(req.params.wishlistId), router.db);
  ...
  const books = db
    ...
    .filter(
      (r) =>
        ...
        (req.query.author ? r.author.toLowerCase().includes(req.query.author.toLowerCase()) : true),
    ).value();
